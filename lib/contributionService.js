...
//      userData.password = '';   // an undefined password can never be authenticated against except for the reset password flow
      userData.memberType = pledgeData.seedcoopInterest ? Profile.MEMBERSHIP_TYPES.provisional : Profile.MEMBERSHIP_TYPES.visitor;
      console.log(`auto signup of new user: ${_.inspect(userData)}`);
      return UserService.createUser(userData);
    }
...
function apiFetchStatus(contributionId) {
  return fetch(contributionId)
    .then((contribution) => {
...
function endRecurringContribution(contributionId) {
  let contribution;
  return fetch(contributionId)
    .then((aContribution) => {
      if (!aContribution) {
      ...
      if (contribution.status !== 'recurring') {
        ...
        return save(contributionId, updateData);
      }
    }).then((result) => {
