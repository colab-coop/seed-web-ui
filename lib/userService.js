...
function login(req, user) {
  return new Promise(function (resolve, reject) {
    req.login(user, function (err) {
      ...
      req.user = user;  // patch current request state
      resolve(user);
    });
  });
}
...
function createUser(data) { //email, password, firstName, lastName, orgName, memberType) {
  ...
  return encryptPassword(data.password)
    ...
    .then((status) => {
      ...
      return sendSystemNotification(resultUser, 'newUser');
    })
    .then((status) => {
      console.log(`sendSystemNotification status: ${status}`);
      return resultUser;
    })
    .catch((err) => {
...
function sendSystemNotification(user, action) {
  const to = mailer.systemLogEmail();
  const link = `${mailer.rootUrl()}/admin/user/${user._id}/view`;
